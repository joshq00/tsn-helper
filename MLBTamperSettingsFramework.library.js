...
if(md5(settings.superSecret) == '2c3005677d594560df2a9724442428d1' || md5(settings.superSecret) == '68839b25c58e564a33e4bfee94fa4333') {
   ...
   schema.properties = Object.assign({}, schema.properties, {
        ...
        "refreshMarketInterval": {
           "type": "integer",
            "title": "Refresh Community Market Interval (seconds)",
       ...
       "showBuyFrame": {
           "type": "boolean",
            ...
            "description": "Show the iframe for buy/sell/cancel orders on Community Market",
            "default": false,
            "required": true
            },
       "ignoreSoloBuySell": {
           "type": "boolean",
            "title": "Ignore single buy/sells",
...
(function() {
    ...
    var closeButton = doc.createElement('button');
       closeButton.innerHTML = 'Close without Saving';
    ...
    if(localStorage.hasOwnProperty('tsn-versionUpdate')){
        updateMessageSeen = localStorage.getItem('tsn-versionUpdate');
    }
    else{
        localStorage.setItem('tsn-versionUpdate', 0);
    }
    if ( version_gt(currentVersion, updateMessageSeen)) {
        ...
        var updateInnerHTML = `<h3>Changelog for v${currentVersion}</h3><ul>`;
        for ( var change of changelog[currentVersion] ) {
        ...
        $( updateDialog ).dialog({
            ...
            buttons: {
                "Got It!": function() {
                    localStorage.setItem('tsn-versionUpdate', currentVersion);
                    $( this ).dialog( "close" );
                },
                Cancel: function() {
                    $( this ).dialog( "close" );
